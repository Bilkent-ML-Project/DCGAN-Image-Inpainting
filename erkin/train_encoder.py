...
class EncoderTrainer(object):
    ...
    def __init__(self, train_params, ckpt_params, gan_params):
        ...
        self.gan = DCGAN(gan_params['gan_type'], gan_params['latent_dim'], 
                         train_params['batch_size'], train_params['use_cuda'])
    ...
    def train_encoder(self, data_loader, val_loader, nb_epochs):
        ...
        for epoch in range(nb_epochs):
            for i, (x, _) in enumerate(data_loader):
                ...
                if i % 100 == 0:
                    logging.info(f'Epoch [{epoch+1}/{nb_epochs}], Step [{i+1}/{len(data_loader)}], Loss: {loss.item()}')
...
if __name__ == '__main__':
    ...
    train_dataset, val_dataset, test_dataset = utils.load_dataset(train_params['root_dir'], train_params['batch_size'])

    data_loader = DataLoader(train_dataset, batch_size=train_params['batch_size'], shuffle=True)
    ...
    encoder_trainer.save_encoder_state(os.path.join(ckpt_params['ckpt_path'], 'encoder.pt'))
