...
def format_hdr(gan, root_dir, training_len):
    ...
    dataset = "Training on CelebA dataset ({}) with {:,} faces".format(
        root_dir, training_len
    )
    hdr = "\n".join(
        [sep_, title, sep, dataset, type_str, loss_str, param_G_str, param_D_str, sep_]
    )
    print(hdr)
...
def progress_bar(batch_idx, report_interval, G_loss, D_loss):
    ...
    print(
        "\rBatch {:>4d} [{}{}] G loss: {:>7.4f} | D loss: {:>7.4f}".format(
            batch_idx, "=" * fill, " " * (bar_size - fill), G_loss, D_loss
        ),
        end="",
    )
...
def show_learning_stats(batch_idx, num_batches, g_loss, d_loss, elapsed):
    ...
    print(
        "Batch {:>{dec}d} / {:d} | G loss: {:>7.4f} | D loss: {:>7.4f} | Avg time / batch: {:d} ms".format(
            batch_idx, num_batches, g_loss, d_loss, int(elapsed), dec=dec
        )
    )
...
def load_dataset(root_dir, batch_size):
    ...
    train_data = ImageFolder(
        root=root_dir, transform=transforms.Compose([transforms.ToTensor(), normalize])
    )
    subset_indices = torch.randperm(len(train_data))[: int(0.1 * len(train_data))]
    subset_dataset = torch.utils.data.Subset(train_data, subset_indices)
    # Assuming subset_dataset is your original dataset
    ...
    num_val = int(val_ratio * dataset_size)
    ...
    train_dataset, val_dataset, test_dataset = random_split(
        subset_dataset, [num_train, num_val, num_test]
    )
