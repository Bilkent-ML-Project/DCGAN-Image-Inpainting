...
def process_image(image_path, square_size=20):
    # Load image
    ...
    image_cutout[
        upper_left_y : upper_left_y + square_size,
        upper_left_x : upper_left_x + square_size,
    ] = 0
...
def show_im(in_im):
    ...
    plt.imshow(
        im.transpose(1, 2, 0)
    )  # Transpose to (height, width, channels) for RGB image
    plt.show()
